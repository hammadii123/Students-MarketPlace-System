...
function displayBooks(books) {
    ...
    booksGrid.innerHTML = books.map((book) => {
        ...
        <div class="col-6 text-start"><h5 class="">${book.title}</h5><h6 class="">Author:${book.author}</h6><p class=""><b> Description </b><br> ${book.description}</p><div class="d-flex justify-content-between align-items-center my-2"> <span>Rs ${book.price}</span>
        <button class="btn btn-dark" onclick="addFavBook(${book.id})">Add to favourite</button></div></div>
        <div></div></div></div>`
    }).join('')
}
if (AllBooksBtn) {
...
async function showFavBooks(Data) {
    ...
    <div class="col p-3 p-lg-4">
        <div class="card-book style="box-shadow: rgba(50, 50, 93, 0.25) 0px 50px 100px -20px, rgba(0, 0, 0, 0.3) 0px 30px 60px -30px, rgba(10, 37, 64, 0.35) 0px -2px 6px 0px inset;">
